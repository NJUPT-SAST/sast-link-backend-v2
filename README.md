...
当前仓库已完成 Go 服务骨架、数据基础层、认证基础设施与首个内部认证闭环：HTTP API 入口、PostgreSQL/Redis 连接、健康检查、V001–V003 SQL migrations（含固定内置 `sast-link-web` first-party Client）、持久化实体与 Auth repositories、密码哈希、RS256 JWT/JWKS、opaque Refresh Token、PKCE-S256、统一 OAuth/OIDC scope、Redis 一次性状态/限流/登录失败计数，以及密码登录、Token 刷新、登出、JWT middleware 和当前用户资料查询。注册/验证码/密码管理、资料编辑、OAuth/OIDC endpoints、设备管理与 pg_cron 运维任务仍待接入。
