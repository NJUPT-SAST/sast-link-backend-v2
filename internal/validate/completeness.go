...
// write path (PUT /user/profile) would refuse: blank, over the column width, or
// containing a C0/C1 control character. The rule mirrors buildProfileUpdate's
// required-field checks so the completion report and the write validation line
// up: an account must be told to fix exactly the values an edit would reject.
//
// This is the Go half of the V010/V015 generated-column rule. Keep it in
// lockstep with the SQL expression and with the limits in limits.go —
