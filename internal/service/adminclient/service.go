...
type Service struct {
	...
	Clock       Clock
	// ProtectedClientIDs is the set of first-party client IDs that Link treats as
	// internal. They cannot be disabled or have redirect_uris rewritten through
	// this service.
	//
	// The built-in client must remain present in this list. Disabling it is an
	// unrecoverable lockout: session.findInternalClient resolves it through an
	// is_active filter, so the flag flip breaks login, refresh and registration for
	// everyone — including the administrator who would flip it back — and the same
	// call revokes every internal session token. Disabling an additional trusted
	// client instead breaks its dependent application; redirect rewriting can divert
	// that application's authorization codes in either case.
	//
	// Empty disables the guard, which is why cmd/api always sets it from the
	// configured internal-client allow-list rather than relying on a default here.
...
// checkProtected refuses an update that would break a trusted internal client.
//
...
// lockout described on ProtectedClientIDs, and the second would let an
...
func (s Service) checkProtected(current *model.OAuthClient, input UpdateClientInput) error {
	if current == nil || !containsClientID(s.ProtectedClientIDs, current.ClientID) {
		return nil
	}
	if input.IsActive != nil && !*input.IsActive {
		return newError(ErrProtectedClient, "受信任内部客户端不可停用", nil)
	}
	if input.RedirectURIs != nil {
		return newError(ErrProtectedClient, "受信任内部客户端的 redirect_uris 不可通过本接口修改", nil)
	}
	return nil
}
