...
func TestAlumniRequestPendingUniquenessAllowsResubmitAfterRejection(t *testing.T) {
...
func TestAlumniRequestApprovalProvisionsAndRecordsTheVerdict(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
...
func TestAlumniRequestApprovalProducesACompleteProfile(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
...
func TestAlumniRequestApprovalWithABlankMajorWouldBeFlagged(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
...
func TestAlumniRequestConcurrentApprovalsYieldOneAccount(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
	reviewer := mustUserID(t, database)
...
func TestAlumniRequestApprovalRollsBackOnProvisionFailure(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
...
func TestAlumniRequestSurvivesTheAccountItCreated(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
	...
	if err := database.Where("id = ?", *approved.CreatedUserID).
...
func TestAlumniRequestNotificationStateWrites(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
	...
	now := time.Now().UTC()
	if err := requests.MarkNotified(ctx, request.ID, now); err != nil {
...
func TestAlumniRequestUpdatedAtAdvances(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
...
func TestRetentionSweepsOnlyReviewedAlumniRequests(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
...
func TestAlumniRequestListFilters(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
	reviewer := mustUserID(t, database)
	...
	rows, _, err = requests.List(ctx, repository.AlumniRequestFilter{Keyword: "王小明", Limit: 10})
	if err != nil {
		t.Fatalf("List() by keyword error = %v", err)
	}
	if len(rows) != 1 || rows[0].ID != first.ID {
		t.Fatalf("keyword filter returned %d rows, want the matching one", len(rows))
	}

	// Neither has been notified yet, so the un-notified filter sees both.
	notified := false
	_, total, err = requests.List(ctx, repository.AlumniRequestFilter{Notified: &notified, Limit: 10})
	if err != nil {
	...
	if err := requests.MarkNotified(ctx, second.ID, time.Now().UTC()); err != nil {
		t.Fatalf("MarkNotified() error = %v", err)
	}
	_, total, err = requests.List(ctx, repository.AlumniRequestFilter{Notified: &notified, Limit: 10})
	if err != nil {
...
func TestAlumniRequestListEscapesLikeMetacharacters(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
...
func TestAlumniRequestRejectRefusesADecidedTicket(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
	ctx := context.Background()
	reviewer := mustUserID(t, database)
...
func TestAlumniRequestGetReportsNotFound(t *testing.T) {
	database := setupDatabase(t)
	requests := repository.NewAlumniRequest(database)
...
func mustUserID(t *testing.T, database *gorm.DB) int64 {
	t.Helper()
	reviewer := testUser("reviewer-alumni@sast.fun")
	reviewer.StudentID = "REVIEWER" + time.Now().Format("150405.000000")
