...
//
// This lives in the repository layer because three services need the same
// classification — the console's user writes, self-service identity binding, and
// third-party OAuth binding — and it is pure inspection of a driver error, not
// policy. Each service still maps the returned name onto its own error type and
