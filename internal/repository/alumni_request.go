...
// DeleteReviewedAlumniRequests removes tickets reviewed before cutoff.
//
// Only approved and rejected tickets are swept. A pending ticket is never deleted
// however old it is: the three-day service target is a statement in the UI, not a
// rule the backend enforces, and dropping an unreviewed application would lose
// someone's request rather than expire it.
func (r *AlumniRequestRepository) DeleteExpiredAlumniRequests(
	ctx context.Context,
	cutoff time.Time,
	batchSize int,
) (int64, error) {
	return r.deleteBatch(ctx, cutoff, batchSize)
}

// deleteBatch bounds the delete to batchSize primary keys, like
// RetentionRepository.deleteBatch: this runs on the worker's ticker while request
// traffic shares the pool, so an unbounded delete on a long-uncleaned table would
// hold row locks for the length of one statement.
func (r *AlumniRequestRepository) deleteBatch(
	ctx context.Context,
	cutoff time.Time,
	batchSize int,
) (int64, error) {
	if cutoff.IsZero() || batchSize <= 0 {
		return 0, fmt.Errorf("cleanup reviewed alumni requests: %w", ErrInvalidArgument)
	}
	...
	result := r.database.WithContext(ctx).
		...
		Delete(&model.AlumniRequest{})
	if result.Error != nil {
		return 0, fmt.Errorf("cleanup reviewed alumni requests: %w", result.Error)
	}
