...
| `40902` | 409 | 学号已有账号（provision 提交时；响应文案引导切换为 `recover` 或联系 support） |
| `40905` | 409 | 目标账号 `other_mail` 绑定数量已达上限（recover 审批时） |
| `40906` | 409 | 该邮箱已有待审申请（任何 intent 下同一邮箱只能排一张单）；或该学号已有待审申请；已驳回的申请不占名额，可修正后重新提交 |
| `42200` | 422 | recover 审批时发现工单 `login_email` 与目标账号不一致，或目标账号已注销——请驳回后让申请人重新提交 |
