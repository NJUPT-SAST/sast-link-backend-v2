...
| `40105` | 邮箱或密码错误（登录统一返回，避免枚举已注册邮箱；旧密码校验、绑定 step-up 的"密码错误"亦为此码） |
...
>
> **step-up 复核**：绑定把一个新登录方式挂到账号名下，请求体必须携带当前账号密码（`password`，JSON）做二次确认——光有被窃取的 Bearer token 不能单独完成绑定。密码错返回 `40105`（401）并记录 `oauth_bind` 失败审计；限流按调用者，60s 内最多 `RATE_LIMIT_PASSWORD_STEPUP_RPM`（默认 10）次且先于密码校验生效。
>
...
**说明**: Bind-Ticket 存储在 Redis，有效期 5 分钟，一次性使用，内部携带待绑定邮箱地址。发送与确认都要携带当前密码做 step-up 复核（见 §4.2），密码错返回 `40105`。
...
| `student_id` | 是 | 同一学号同时只允许一条待审申请 |
| `login_email` | 是 | 原学号邮箱，将成为账号登录身份，仍限 `@njupt.edu.cn` / `@sast.fun` |
...
| 业务码 | HTTP | 场景 |
|--------|------|------|
| `40000` | 400 | 字段缺失/超长/含控制字符、未知字段、Content-Type 非 JSON、`name` 与 `student_id` 相同、`personal_email` 与 `login_email` 相同 |
...
| `40902` | 409 | 学号已有账号 |
| `40906` | 409 | 该学号已有待审申请；已驳回的申请不占用名额，可修正后重新提交 |
...
**错误码**：`40100`、`40300`、`40403`（申请不存在）、`40901`/`40902`（提交后到审批之间被他人抢注，属竞态）、`42204`（申请已被处理）、`50000`。
