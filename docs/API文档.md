...
`50301` 与 `40021` 必须区别对待，二者含义相反：`40021` 表示人机校验**跑过但没通过**，客户端应让用户重新完成验证后重试；`50301` 表示校验**根本没能进行**（未配置 `TURNSTILE_SECRET`，或 Cloudflare siteverify 超时/不可达），此时重做验证毫无意义，客户端应隐藏申请入口。把后者报成前者会让用户反复去解一个本来正常工作的验证组件。

免登录写端点在校验不可用时**拒绝而非放行**：`POST /alumni-requests` 是本服务唯一的未认证写入口，关掉人机校验只剩限流兜底，宁可短暂不可用。
...
| `name` | 是 | 姓名（仅汉字与间隔号 `·` 及其常见变体，规则见 §3.0 说明） |
| `phone_number` | 是 | 手机号 |
| `qq_number` | 是 | QQ 号 |
| `student_id` | 是 | 学号 |
...
**错误码**: 400xx（参数错误）、40105（密码错误）、40106（邮箱不存在）、40301（账号已注销）、429xx（频率限制与登录锁定）；不存在账号的尝试同样计入失败锁定（按邮箱维度计数）
...
**说明**: 邮箱不存在时返回 `40106`（邮箱不存在）。存在的账号仍为异步受理：响应只表示请求已入队，不表示邮件已经送达。服务端把请求放入有界内存队列；队列满、进程重启或邮件依赖失败时任务可能丢失，用户可在限流窗口后重试。
...
**错误码**: 400xx（参数错误）、40106（邮箱不存在）、429xx（频率限制）
...
- 不是权限输入，不参与任何鉴权判断。
- 只读。该列是 PostgreSQL 生成列（V010 初建 / V015、V016 重建），用户通过 `PUT /user/profile`（§3.2）补齐字段后自动转为 `false`，无专门的「确认已补全」接口。判定与写路径的拒绝形状完全对齐：任何被 `PUT /user/profile` 拒绝的值（空白 / 超长 / C0/C1 控制字符，`name` 另有字符集规则与学号重名——**字符集规则与前端口径一致**：仅汉字与间隔号 `·` 及其常见变体（U+00B7 / U+30FB / U+FF65 / U+2027 / U+0387），即注册与编辑表单的 `realNameSchema`，后端全部写 `name` 的入口同规则拒绝）都会让对应字段留在 `incomplete_fields` 里，因此待补全账号总是能凭页面提示找到并修掉不可用值（旧版只判空白与学号重名，带控制字符的脏字段会从提示里消失，补完其他字段后该列翻回 `false`，残渣永久留存；再往前一版连字符集之外的 `"AAA"` 这类名字都不标记，编辑被整表单校验锁死且无引导）。
...
`initial_password` 是系统生成的随机初始密码，仅在本次响应中返回，不落库也不入审计。管理员需线下转达给成员；成员首次可凭 `login_email`（或绑定后的 `personal_email`）+ 初始密码登录，随后经「修改密码」或「忘记密码」更换。
...
- `provision`（缺省）——为新学号开一个新账号；
- `recover` ——**恢复已有账号的访问方式**：审批通过不给该学号再开新号，而是把申请中的个人邮箱直绑到该学号现有账号上（同一事务内），绑定后即可走重置密码流程重新登录。提交时要求 `login_email` 与该学号现有账号登记的登录邮箱一致；审批时在同一事务内再次核对。
...
本服务**唯一的未认证写端点**，由 Cloudflare Turnstile 人机校验 + 双维度限流（按 IP、按学号）把守。

**Headers**: `Content-Type: application/json`（不需要 `Authorization`）
...
{
  "name": "张三",
  "student_id": "B20040525",
  "login_email": "b20040525@njupt.edu.cn",
  "personal_email": "zhangsan@qq.com",
  "phone_number": "13800138000",
  "qq_number": "12345",
  "college": "计算机学院、软件学院、网络空间安全学院",
...
| `college` | 否 | 缺省 `其他` |
| `join_year` | 是 | 入部年份，≤ 32 字符 |
| `department_note` | 否 | 所属部门备注，≤ 255 字符 |
| `note` | 否 | 补充说明，≤ 1000 字符 |
...
只回工单 id。提交者未经认证，响应不透露除下列冲突码之外的任何账号存在性信息。
...
| `40906` | 409 | 该邮箱已有待审申请（任何 intent 下同一邮箱只能排一张单）；或该学号已有待审申请；已驳回的申请不占名额，可修正后重新提交 |
...
- `intent` 是该工单要的动作：`provision` 开新号、`recover` 恢复已有账号访问。**recover 卡片必须以高危操作呈现**：审批前展示目标账号的现有资料与绑定，审批触达的是现役账号而非新建
...
```
GET /admin/alumni-requests/:id
```

**Headers**: 同 §6.13.2

字段结构同列表项。`:id` 非正整数或不存在均返回 `40403`。

**错误码**：`40100`、`40300`、`40403`、`50000`。
...
**原子性**：锁工单行（`SELECT ... FOR UPDATE`）→ 建号（或绑邮箱，见下）→ 回写审批结果，三者同一事务。任一步失败整体回滚，工单留 `pending`，管理员可改字段重试。控制台按钮被双击时，第二个请求读到已是 `approved`，返回 `42204` 而不会重复建号。

**recover 工单的审批**：不建号、不生成密码。同一事务内按 `lower(btrim(student_id))` 定位并锁定现有账号 → 核对 `login_email` 一致 → 把工单个人邮箱直绑为该账号的 `other_mail` 身份 → 回写审批结果（`created_user_id` 记录的是现有账号）。响应形状不变（`user_id` 即恢复访问的账号）。目标账号已注销返回 `42200`；绑定数量已达上限返回 `40905`；锁定后核对发现 `login_email` 不一致也返回 `42200`（提交预检之后的漂移）。审计只记一行审批结果（detail 带 `recovered/target_user_id/bound_email`），不写开户行——没有账号被创建。

**错误码**：`40100`、`40300`、`40403`（申请不存在）、`40901`/`40902`（提交后到审批之间被他人抢注，属竞态）、`40905`（目标账号绑定数量已达上限）、`42200`（recover 目标已注销或 `login_email` 漂移）、`42204`（申请已被处理）、`50000`。
...
```json
{ "reject_reason": "学号与姓名不匹配，请核对后重新提交" }
```
...
```
POST /admin/alumni-requests/:id/resend-notification
```
