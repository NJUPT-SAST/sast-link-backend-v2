...
-- OAuth 标准客户端安全分类
CREATE TYPE client_oauth_type_enum AS ENUM (
    'confidential', -- 能安全保存 client_secret（如后端服务）
    'public'        -- 不能安全保存 secret（如 SPA、移动端）
);
```
...
|name|姓名|
|phone_number|手机号，应用层 AES-256-GCM 加密后存储|
|qq_number|QQ号，应用层 AES-256-GCM 加密后存储|
|password|密码，argon2id 哈希存储；OAuth 纯登录用户可为 NULL|
|student_id|学号，应用层 AES-256-GCM 加密后存储|
|state|状态机：`on_sast` / `retired_sast` / `njupter`|
|email_type|邮箱种类（方便后续添加）|
|login_email|用于注册的邮箱|
|token_version|安全版本号，改密后递增，使所有 Token 失效|
|is_deleted|账号是否已注销（软删除标记）|
|deleted_at|注销时间，NULL = 未注销|
...
CREATE INDEX idx_profile_custom_urls
ON profile USING GIN(custom_urls);
```
...
|custom_urls|自定义URL，JSONB数组，最多3条|
|created_at|首次创建时间|
|updated_at|最后更新时间|
...
CREATE TABLE audit_logs (
    ...
    CONSTRAINT valid_action
        CHECK (action IN (
            ...
            'oauth_bind', 'oauth_unbind', 'update_profile', 'upload_avatar',
            'admin_action'
        ))
);
```
...
CREATE TABLE oauth_clients (
    ...
    client_secret     VARCHAR(255),          -- 第一方应用 / public 客户端存 NULL
    client_name       VARCHAR(255)           NOT NULL,
    client_type       client_enum            NOT NULL,
    client_oauth_type client_oauth_type_enum NOT NULL,
    ...
    CONSTRAINT ck_oauth_clients_secret_consistency
        CHECK (
            (client_oauth_type = 'confidential' AND client_secret IS NOT NULL) OR
            (client_oauth_type = 'public' AND client_secret IS NULL)
        )
);
```
...
|`client_name`|应用名称|
|`client_type`|第一方应用 / 第三方应用（业务分类）|
|`client_oauth_type`|`confidential` / `public`（OAuth 安全分类）|
...
|`code_challenge_method`|`S256`，CHECK 约束，NOT NULL|
|`nonce`|OIDC nonce|
|`is_used`|是否已使用|
...
## user_consents 用户授权同意记录表

OAuth 2.1 授权流程中，用户确认授权后记录授权范围、时间，用于后续查看/撤销已授权应用。
...
CREATE TABLE user_consents (
    ...
    CONSTRAINT uq_user_consents_user_client
...
DECLARE
    mail_count INT;
BEGIN
    -- INSERT 场景：新增 other_mail
    IF TG_OP = 'INSERT' AND NEW.provider = 'other_mail' THEN
    ...
    -- UPDATE 场景：将其他 provider 改为 other_mail
    IF TG_OP = 'UPDATE' AND OLD.provider != 'other_mail' AND NEW.provider = 'other_mail' THEN
        SELECT COUNT(*) INTO mail_count
        FROM identities
        WHERE user_id = NEW.user_id AND provider = 'other_mail';
...
RETURNS TRIGGER AS $$
DECLARE
    expected_type email_enum;
BEGIN
    IF LOWER(NEW.login_email) LIKE '%@sast.fun' THEN
        expected_type := 'sast_email';
    ELSIF LOWER(NEW.login_email) LIKE '%@njupt.edu.cn' THEN
        expected_type := 'njupt_email';
    ELSE
        RAISE EXCEPTION 'Invalid email domain: %. Only @njupt.edu.cn and @sast.fun are allowed.', NEW.login_email;
    END IF;

    -- 仅在 email_type 为空或与预期不符时覆盖，避免静默覆盖应用层显式值
    IF NEW.email_type IS NULL OR NEW.email_type != expected_type THEN
        NEW.email_type := expected_type;
    END IF;
...
> `check_other_mail_limit` 已覆盖 INSERT 和 UPDATE 两种场景。
> `auto_set_email_type` 仅在 `email_type` 为空或与预期不符时覆盖，避免静默覆盖应用层显式值。
...
```sql
-- 已过期且未使用的授权码（每小时）
DELETE FROM oauth_authorizations WHERE is_used = FALSE AND expires_at < NOW() - INTERVAL '1 hour';
...
10. `user_consents` 表（FK → user, oauth_clients）
