...
SAST Link Backend V2 is the backend for SAST's unified identity/authentication center and personnel information system. The current implementation establishes the Go module, HTTP service skeleton, V001–V004 PostgreSQL schema migrations (including the built-in `sast-link-web` first-party client and token-blacklist outbox), persistence entities and Auth repositories, authentication primitives, and the first internal session flow: password login, refresh rotation/replay handling, logout, JWT middleware, durable Redis blacklist delivery, and current-user profile retrieval. PostgreSQL 16 and Redis integration tests use Testcontainers. Registration/password management, profile mutation, third-party login, OAuth/OIDC provider endpoints, device management, and pg_cron operations remain to be implemented.
